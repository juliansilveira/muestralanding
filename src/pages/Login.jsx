import { useState } from "react";
import { Link, useNavigate } from "react-router-dom";
import api from "../services/api.js"; // Importa tu servicio api
import { useAuth } from "../components/context/AuthContext.jsx"; // Importa el contexto de autenticación
import Logo from "../assets/Iconos GestionaApp/Logo.svg";

export default function Login() {
  const [formData, setFormData] = useState({
    correo: "",
    clave: "",
  });

  // Asegúrate de declarar TODOS los estados necesarios
  const [errorMessage, setErrorMessage] = useState(""); // <-- ESTADO FALTANTE
  const [isLoading, setIsLoading] = useState(false);

  const navigate = useNavigate();
  const { login } = useAuth();

  const handleChange = (e) => {
    setFormData({ ...formData, [e.target.name]: e.target.value });
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    setErrorMessage(""); // Limpiar errores anteriores

    try {
      const response = await api.post("/iniciar-sesion", formData);

      if (!response.data.token || !response.data.idUsuario) {
        throw new Error("La respuesta del servidor es inválida");
      }

      // Guardar token y datos de usuario
      localStorage.setItem("token", response.data.token);
      localStorage.setItem(
        "usuario",
        JSON.stringify({
          idUsuario: response.data.idUsuario,
          nombre: response.data.nombre,
          correo: response.data.correo,
          estadoSuscripcion: response.data.estadoSuscripcion,
          fechaFinPrueba: response.data.fechaFinPrueba
        }),
      );

      // Actualizar estado de autenticación
      login();

      // Redirigir al Login
      navigate("/home");
    } catch (error) {
      let message = "Error de conexión con el servidor";

      if (error.response) {
        if (error.response.data && error.response.data.mensaje) {
          message = error.response.data.mensaje;
        } else if (error.response.status === 401) {
          message = "Credenciales incorrectas";
        } else if (error.response.status === 500) {
          message = "Error interno del servidor";
        }
      } else if (error.request) {
        message = "El servidor no respondió";
      } else {
        message = error.message;
      }

      setErrorMessage(message); // Usando el estado declarado
    } finally {
      setIsLoading(false);
    }
  };

  return (
    <div className="flex flex-col items-center justify-center bg-bg-login min-h-screen w-full font-poppins p-4">
      {/* ——— Popup de éxito ——— */}

      <div className="max-w-md w-full space-y-6 md:space-y-8 lg:space-y-10">
        <div className="flex justify-center">
          <img src={Logo} alt="Logo GestionaApp" className="mx-auto" />
        </div>

        <div className="rounded-xl p-6 md:p-8 lg:p-10 space-y-4 md:space-y-6">
          <h1 className="text-2xl md:text-3xl font-bold text-white text-center mb-4">
            Iniciar Sesión
          </h1>

          {errorMessage && (
            <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
              {errorMessage}
            </div>
          )}

          <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
            <div>
              <label className="block text-white text-md md:text-base mb-1">
                Correo Electrónico
              </label>
              <input
                type="email"
                name="correo"
                value={formData.correo}
                onChange={handleChange}
                className="w-full p-2 md:p-3 rounded-full border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                disabled={isLoading}
                placeholder="Correo Electrónico"
              />
            </div>

            <div className="">
              <label className="block text-white text-sm md:text-base mb-1 flex justify-between">
                Contraseña
                <Link
                  to="/recuperar-clave"
                  className="text-white text-xs md:text-sm hover:underline cursor-pointer"
                >
                  ¿Olvidaste tu contraseña?
                </Link>
              </label>
              <input
                type="password"
                name="clave"
                value={formData.clave}
                onChange={handleChange}
                className="w-full p-2 md:p-3 rounded-full border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                disabled={isLoading}
                placeholder="Contraseña"
              />
            </div>

            {/* Separador */}
            <div className="flex items-center my-4">
              <div className="flex-1 border-t border-gray-300"></div>
              <span className="px-4 text-gray-500 text-sm">o</span>
              <div className="flex-1 border-t border-gray-300"></div>
            </div>

            {/* Botón de Google */}
            <button
              type="button"
              onClick={() => {
                window.location.href = "http://localhost:3010/api/auth/google";
              }}
              className="w-full bg-white text-gray-700 py-3 px-4 rounded-full
                        border-2 border-gray-300 hover:border-gray-400 hover:bg-gray-50
                        transition-colors duration-200 font-medium text-base
                        flex items-center justify-center gap-3"
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24">
                <path
                  fill="#4285F4"
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                />
                <path
                  fill="#34A853"
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                />
                <path
                  fill="#FBBC05"
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                />
                <path
                  fill="#EA4335"
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                />
              </svg>
              Continuar con Google
            </button>

            <button
              type="submit"
              className="w-full bg-rosadoFuerte text-white py-3 md:py-3 px-4 rounded-full
                          hover:bg-fondoRosaClaro hover:text-black transition-colors duration-200 font-bold
                          text-lg md:text-lg flex justify-center items-center"
              disabled={isLoading}
            >
              {isLoading ? (
                <>
                  <svg
                    className="animate-spin -ml-1 mr-3 h-5 w-5 text-white"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
                    <circle
                      className="opacity-25"
                      cx="12"
                      cy="12"
                      r="10"
                      stroke="currentColor"
                      strokeWidth="4"
                    ></circle>
                    <path
                      className="opacity-75"
                      fill="currentColor"
                      d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"
                    ></path>
                  </svg>
                  Procesando...
                </>
              ) : (
                "Ingresar"
              )}
            </button>
          </form>
        </div>
        <p className="text-gray-500 text-center text-sm md:text-base">
          ¿No tienes cuenta?{" "}
          <Link to="/registro">
            <span className="text-blue-600 hover:underline">Regístrate</span>
          </Link>
        </p>
      </div>
    </div>
  );
}
